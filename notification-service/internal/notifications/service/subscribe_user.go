...
func (s *NotificationService) SubscribeUser(ctx context.Context, userId, endpoint, p256dh, auth string) error {
	...
	l.Infof("new subscriber userid: %s", userId)
	l.Infof("new subscriber endpoint: %s", endpoint)
	l.Infof("new subscriber p256: %s", p256dh)
	l.Infof("new subscriber auth: %s", auth)
